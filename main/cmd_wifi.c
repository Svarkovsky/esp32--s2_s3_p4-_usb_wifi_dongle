 ...
 * - Auto-SmartConfig fallback (4s timeout)
 * - Wi-Fi credentials saved to FLASH (NVS)
 * - RGB LED status indication (WS2812/SK6812)
 * - Zero-console Wi-Fi setup via ESP Touch
 */
...
// --- ДОБАВЛЕНО: Управление адресным светодиодом ---
#include "led_strip.h"

led_strip_handle_t led_strip;

void configure_led(void) {
    int blink_gpio = -1;  // По умолчанию — светодиод отключён

    // Определяем пин в зависимости от целевого чипа
#if CONFIG_IDF_TARGET_ESP32S3
    // DevKitC v1.0: GPIO48, v1.1+: GPIO38
    #ifdef CONFIG_BOARD_ESP32S3_DEVKITC_V11
        blink_gpio = 38;
    #else
        blink_gpio = 48;  // ESP32-S3 for UNO — 48
    #endif
#elif CONFIG_IDF_TARGET_ESP32S2
    // Saola-1R использует GPIO18 для адресного LED
    blink_gpio = 18;
#elif CONFIG_IDF_TARGET_ESP32P4
    // На P4 DevKit обычно нет адресного LED — отключаем
    blink_gpio = -1;
#else
    blink_gpio = -1;  // Неизвестный чип — отключаем
#endif

    // Инициализируем только если пин валидный
    if (blink_gpio >= 0) {
        ...
        // Пробуем инициализировать БЕЗ ESP_ERROR_CHECK — чтобы не упасть
        esp_err_t err = led_strip_new_rmt_device(&strip_config, &rmt_config, &led_strip);
        if (err == ESP_OK) {
            led_strip_clear(led_strip);
            ESP_LOGI("LED", "Addressable LED initialized on GPIO %d", blink_gpio);
        } else {
            ESP_LOGW("LED", "LED init failed on GPIO %d (err=%d). Working without indication.", blink_gpio, err);
            led_strip = NULL;
        }
    } else {
        ESP_LOGI("LED", "Addressable LED not available for this target chip");
        led_strip = NULL;
    }
}
                               ...
                               int32_t event_id, void *event_data)
{
    ...
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        ...
        // Раскомментировано для автоподключения ---
        esp_wifi_connect(); 
        ...
        // При обрыве связи включаем КРАСНЫЙ ---
        ...
        if (smart_config == false) {
            ...
            // При успешном подключении ЗЕЛЕНЫЙ ---
        ...
        // Успешное завершение SmartConfig (ЗЕЛЕНЫЙ) ---
        set_led_color(0, 20, 0); 
    }
#endif
    free(wifi_event_buf);
...
esp_err_t wifi_cmd_start_smart_config(void)
{
    if (wifi_start) {
        ...
        // Синий цвет при ожидании SmartConfig ---
...
void initialise_wifi(void)
{
    ...
    // Запуск светодиода и Красный цвет при старте ---
    configure_led();
    set_led_color(20, 0, 0); 
    // -------------------------------------------------------------
    ...
    // Запись пароля навсегда во FLASH память ---
