...
void app_main(void)
{
    ...
    // --- (SmartConfig) ---
    // Ждем 4 секунды. Если плата не смогла подключиться к сохраненной сети,
    // значит она новая или пароль сменился. Включаем SmartConfig.
    vTaskDelay(4000 / portTICK_PERIOD_MS); 
    
    wifi_ap_record_t ap_info;
    if (esp_wifi_sta_get_ap_info(&ap_info) != ESP_OK) {
        ESP_LOGW("DONGLE", "No Wi-Fi connection. Starting SmartConfig...");
        wifi_cmd_start_smart_config();
    } else {
        ESP_LOGI("DONGLE", "Wi-Fi already connected!");
    }
    // -------------------------------------
